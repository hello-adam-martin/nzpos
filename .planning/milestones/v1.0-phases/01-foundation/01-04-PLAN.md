...
<task type="auto">
  ...
  <files>src/actions/auth/ownerSignup.ts, src/actions/auth/ownerSignin.ts, src/actions/auth/staffPin.ts, src/lib/supabase/admin.ts</files>
  <read_first>
  ...
  <acceptance_criteria>
    ...
    - src/actions/auth/ownerSignup.ts creates store and staff records using admin client
    - src/actions/auth/ownerSignup.ts contains "supabase.auth.refreshSession()" after staff record creation
    ...
    - src/actions/auth/staffPin.ts SELECT clause includes 'name' field
  ...
  <done>Owner can sign up (creates auth user + store + staff record, refreshes session for JWT claims) and sign in. Staff PIN auth verifies bcrypt hash, selects name field, tracks attempts, locks after 10 failures in 5 min, issues 8h jose JWT in httpOnly cookie.</done>
</task>
...
- Owner signup refreshes session after staff record creation
</verification>

<success_criteria>
Owner signup creates auth user + store + staff record and refreshes session for JWT claims. Owner signin works with email/password. Staff PIN auth verifies bcrypt, selects name field, tracks attempts, locks after 10 failures, issues jose JWT. Middleware enforces role-based access on all route groups. Seed script populates 25 realistic NZ products.
</success_criteria>
