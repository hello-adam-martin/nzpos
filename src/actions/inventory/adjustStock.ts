...
export async function adjustStock(input: unknown) {
  ...
  const gate = await requireFeature('inventory' as any, { requireDbCheck: true })
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { data, error } = await (adminClient as any).rpc('adjust_stock', {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  return { success: true as const, new_quantity: (data as unknown as { new_quantity: number }).new_quantity }
}
